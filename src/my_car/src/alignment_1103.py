...
class Alignment():
    def __init__(self):
        # 初始化Publisher
        self.pan_pub = rospy.Publisher('/pan_servo_angle', UInt16, queue_size=10)
        self.tilt_pub = rospy.Publisher('/tilt_servo_angle', UInt16, queue_size=10)
        self.cmd_pub = rospy.Publisher('cmd_vel', Twist, queue_size=10)
        self.elevating_pub = rospy.Publisher('/elevating_height', Float64, queue_size=10)  # 新增這一行
        self.tfbuffer = tf2_ros.Buffer()
        tf2_ros.TransformListener(self.tfbuffer)
        self.tfbcaster = tf2_ros.StaticTransformBroadcaster()
        self.rate = rospy.Rate(10.0) # 提高到 30 Hz
        self.vel = Twist()
    ...
    def set_elevating_height(self, height):
        '''
        設置升降機構的高度
        '''
        height_msg = Float64()
        height_msg.data = height
        self.elevating_pub.publish(height_msg)
        rospy.sleep(1)  # 等待升降機構到達指定高度
    ...
    def set_camera_angles(self, pan_angle, tilt_angle):
        ...
        rospy.sleep(1)  # 等待雲台到達指定角度
    
    def main(self):

        # 在接收導航訊息前設置升降機構的高度
        self.set_elevating_height(0.66)  # 設置高度為660mm

        # 在接收導航訊息前設置相機雲台的角度
        self.set_camera_angles(0, 90)  # 設置水平角度為0度，垂直角度為90度
        ...
        if self.current_sequence is self.AlignmentSequence.wait_reach_goal.value:
            ...
            if self.current_sequence == self.AlignmentSequence.searching_code.value:
                self.is_sequence_finished = self.Searching_Code()
                if self.is_sequence_finished:
                    print("Found AprilTag, stopping...")
                    self.allStop()
                    self.current_sequence = self.AlignmentSequence.set_target_frame.value
        ...
        ############################    
            ...
            rospy.sleep(2)
            self.set_target_frame("tag_1", 0.55)      # Set so that the dummy target frame is 0.3 m front of the AprilTag code
        ...
        ## Moving Forward ##
        ####################
        elif self.current_sequence is self.AlignmentSequence.moving_forward.value:
            print("Moving forward...")
            self.GoStraight(0.1)  # 讓車子向前移動
            try:
                self.listen_tf_frame('/usb_cam', 'next_tag')  # 檢測下一個AprilTag
                self.allStop()
                print("Next tag found!")
                self.current_sequence = self.AlignmentSequence.finished.value  # 或者切換到其他狀態
    ...
    def Searching_Code(self):
        ...
        try:
            ...
            print("AprilTag detected!")  # 調試訊息
            self.allStop()
            return True
        except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
            print("AprilTag not found, rotating...")  # 調試訊息
            return False
    ...
    def listen_tf_frame(self, parent_frame, child_frame):
        ...
        while not rospy.is_shutdown():
            try:
                ...
                print("Fail", e)
                rospy.sleep(5)  # 增加延遲，讓錯誤打印間隔更長
                continue
            self.rate.sleep()
    
    def update_camera_tf(self):
        '''
        # Continuously update the camera's TF based on its current orientation.
        '''
        while not rospy.is_shutdown():
            try:
                # 假設相機的TF變換是從'base_link'到'usb_cam'
                trans = self.listen_tf_frame('base_link', 'usb_cam')
                # 根據相機的當前角度進行更新
                self.camera_transform = trans
            except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException) as e:
                continue
            self.rate.sleep()
